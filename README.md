...
No regex can classify them. Teachers fill the category field about half the time, the highest-stakes post in the set is filed under `Announcements` rather than `Homework`, and the due date is always prose: `HW for Tuesday May 5` puts it in the title, `Solve from May 24, question 2` names a past paper instead of a deadline, and `solve them if you have the time` is not a task at all.
